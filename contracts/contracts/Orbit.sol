...
contract Orbit {
    ...
    function ask(string calldata nlQuery) external payable returns (uint256 queryId) {
        ...
        messages[0] = Prompt.compose();
        messages[1] = string.concat("Query: \"", nlQuery, "\"");

        ILLMAgent.OnchainTool[] memory onchainTools = Tools.getOnchainTools();
    ...
    function handleToolSelection(
        ...
        if (Tools._eq(finishReason, "stop")) {
        ...
        require(Tools._eq(finishReason, "tool_calls"), "Unexpected finish reason");
        ...
        for (uint256 i = 0; i < toolCalls.length; i++) {
            ...
            bool handled = true;
            if (selector == Tools.SWAP) {
                exec.result = Tools.executeSwap(params);
            } else if (selector == Tools.GLOBAL_STATE) {
                exec.result = Tools.executeGlobalState(params);
            } else if (selector == Tools.POOL_LIQUIDITY) {
                exec.result = Tools.executePoolLiquidity(params);
            } else if (selector == Tools.POOL_FEE) {
                exec.result = Tools.executePoolFee(params);
            } else {
                handled = false;
    ...
    function _formatResult(bytes4, bytes memory data) internal pure returns (string memory) {
